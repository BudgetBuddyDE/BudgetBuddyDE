import {ParqetSchemas, type ServiceResponse, type Timeframe} from '@budgetbuddyde/types';
import {z} from 'zod';
...
export class Parqet {
  ...
  public static async getAsset(identifier: string): Promise<ServiceResponse<z.infer<typeof ParqetSchemas.Asset>>> {
    const query = new URLSearchParams();
    query.append('currency', 'EUR');
    query.append('expand', 'details');
    query.append('expand', 'yieldTTM');

    const response = await fetch(`${Parqet.apiHost}/assets/${identifier}?${query.toString()}`);
    if (!response.ok) {
      return [null, new Error(`Failed to fetch asset data for ISIN ${identifier}: ${response.statusText}`)];
    }
    ...
    if (!parsingResult.success) {
      return [
        null,
        new Error(`Failed to parse asset data for ISIN ${identifier}: ${JSON.stringify(parsingResult.error.issues)}`),
      ];
    }
  ...
  public static async getQuotes(
    ...
    const requestBody: {identifier: string; timeframe: string}[] = assets.map(({identifier}) => ({
      identifier,
      timeframe: timeframe,
    }));

    const response = await fetch(`${Parqet.apiHost}/quotes?${query.toString()}`, {
      method: 'POST',
      headers: {
  ...
  public static async getDividends(
    ...
    const response = await fetch(`${Parqet.apiHost}/assets/dividends?${query.toString()}`);
    if (!response.ok) {
  ...
  public static async search(
    ...
    const response = await fetch(`${Parqet.apiHost}/search`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
    ...
    if (!response.ok) {
      return [null, new Error(`Failed to search assets for query ${query}: ${response.statusText}`)];
    }
    const jsonResponse = await response.json();
  ...
  public static async getRelatedAssets(
    ...
    try {
      ...
      const response = await fetch(`${Parqet.apiHost}/assets/${isin}/related?${query.toString()}`);
      if (!response.ok) {
  ...
  public static async getSectors(): Promise<ServiceResponse<z.infer<typeof ParqetSchemas.Sector>[]>> {
    try {
      const response = await fetch(`${Parqet.apiHost}/sectors`);
      if (!response.ok) {
        return [null, new Error(response.statusText)];
      }
  ...
  public static async getRegions(): Promise<ServiceResponse<z.infer<typeof ParqetSchemas.Region>[]>> {
    try {
      const response = await fetch(`${Parqet.apiHost}/regions`);
      if (!response.ok) {
        return [null, new Error(response.statusText)];
      }
  ...
  public static async getIndustries(): Promise<ServiceResponse<z.infer<typeof ParqetSchemas.Industry>[]>> {
    try {
      const response = await fetch(`${Parqet.apiHost}/industries`);
      if (!response.ok) {
        return [null, new Error(response.statusText)];
      }
  ...
  public static async getCountries(): Promise<ServiceResponse<z.infer<typeof ParqetSchemas.Country>[]>> {
    try {
      const response = await fetch(`${Parqet.apiHost}/countries`);
      if (!response.ok) {
        return [null, new Error(response.statusText)];
      }
