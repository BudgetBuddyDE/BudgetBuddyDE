...
export const EtfDetails: React.FC<EtfDetailsProps> = ({
	...
	const KPIs = [
		{
			label: "AUM",
			value: `${Formatter.currency.shortenNumber(aum)} ${currency}`,
		},
		...
		<Card sx={{ p: 0 }}>
			...
			<Card.Body>
				...
				<Box sx={{ px: 2, pb: 2 }}>
					...
					<PieChart
						fullWidth
						primaryText={`${breakdown.holdings
							.reduce((acc, holding) => acc + holding.share, 0)
							.toFixed(2)}%`}
						...
						series={[
							{
								...
								valueFormatter: (value) =>
									Formatter.currency.shortenNumber(value.value),
							},
						]}
					/>
