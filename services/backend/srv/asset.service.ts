...
import { MetalPriceAPI, type SuccessMetalQuoteResponse } from './lib/MetalPriceAPI';
import { MetalCache, AssetCache } from './cache';
import { ParqetSchemas, BackendSchemas, ISIN, AssetIdentifier } from '@budgetbuddyde/types';
...
export class AssetService extends BaseService {
  ...
  async init() {
    this.after('READ', StockPositions, async (positions) => {
      ...
      const uniqueIsins = [...new Set(positions.map((position) => position.isin))].filter(
        (isin): isin is string => Boolean(isin)
      );
      ...
      assetDetailsResults.forEach((result, index) => {
        if (result.status === 'fulfilled' && !result.value.err) {
          ...
          this.logger.error(`Error fetching asset details for ISIN ${isin}: ${error.message}`);
        }
      });

      // Update all positions with the fetched data
      for (const position of positions) {
        if (!position.isin || !position.purchasePrice || !position.quantity) continue;
        ...
        position.positionValue = this.utils.toDecimal(currentPrice * position.quantity);
      }
    });
    ...
    this.after('READ', StockPositionAllocations, async (positions) => {
      ...
      const assetDetailsPromises = uniqueIdentifiers.map(async (isin: string) => {
        const [assetDetails, err] = await Parqet.getAsset(isin);
        return { isin, assetDetails, err };
      });
      const assetDetailsResults = await Promise.allSettled(assetDetailsPromises);
      ...
      const totalPositionsValue = positions.reduce((value, curr) => {
        assert(curr.absolutePositionSize, 'The absolute position size should be defined');
        return value + curr.absolutePositionSize;
      }, 0);

      for (const position of positions) {
        assert(position.absolutePositionSize, 'The absolute position size should be defined');
    ...
    this.on('READ', StockPositionsKPIs, async (req) => {
      ...
      const [assetQuotes, err] = await Parqet.getQuotes(Array.from(assetIdentifiers));
      if (err) {
        req.error(err);
        return;
      }
      ...
      for (const details of Object.values(fetchedDividends.dividendDetails)) {
        ...
        upcomingDividendPayments.set(details.identifier, totalUpcomingDividends);
      }
      ...
      return stockPositions.reduce((stats, position) => {
        ...
        stats[isProfitable ? 'unrealisedProfit' : 'unrealisedLoss'] = this.utils.toDecimal(
          stats[isProfitable ? 'unrealisedProfit' : 'unrealisedLoss'] + Math.abs(capitalGains)
        );
        stats[isProfitable ? 'freeCapitalOnProfitablePositions' : 'boundCapitalOnLosingPositions'] =
          this.utils.toDecimal(
            stats[
              isProfitable ? 'freeCapitalOnProfitablePositions' : 'boundCapitalOnLosingPositions'
        ...
        const dividendPayment = upcomingDividendPayments.has(position.isin)
          ...
          : 0;
        stats.upcomingDividends = stats.upcomingDividends + dividendPayment * position.quantity;
    ...
    this.on('READ', Dividends, async (req) => {
      ...
      if (
        ...
        (typeof reqQuery.identifier === 'string' || Array.isArray(reqQuery.identifier))
      ) {
        if (typeof reqQuery.identifier === 'string') {
          ...
          for (const identifier of reqQuery.identifier) {
            if (typeof identifier !== 'string') {
              this.logger.warn(`Invalid identifier value '%s'. Skipping...`, identifier);
            }
      ...
      if ('historical' in reqQuery) {
        options.historical = reqQuery.historical === 'true' || reqQuery.historical === true;
      }
      ...
      if (!options.future && !options.historical) {
        ...
        req.reject(400, 'At least one of future or historical dividends must be requested');
      }
      ...
      const mergedDividends = Object.entries(fetchedDividends.dividendDetails).flatMap(
        ([, details]) => {
      ...
      return mergedDividends.sort((a, b) => b.paymentDate.getTime() - a.paymentDate.getTime());
    });
    ...
    this.on('READ', SearchAssets, async (req) => {
      ...
      if (!searchQuery || searchQuery.length === 0) {
        this.logger.warn('SearchAssets: No search query provided.', searchQuery);
        return [];
      }
      const searchTerm = searchQuery[0].val;
      this.logger.debug(`SearchAssets: Searching for assets with term "${searchTerm}"`);
      const [searchResults, error] = await Parqet.search(searchTerm);
      if (error) {
        this.logger.error(`Error searching for assets with term "${searchTerm}": ${error.message}`);
        return [];
      }
    ...
    this.after('READ', MetalQuotes, async (metals) => {
      ...
      const symbols = Array.from(new Set(metals.map((metal) => metal.symbol as string)));

      const prices = new Map<string, SuccessMetalQuoteResponse<string>['rates']>();
      await Promise.all(
        symbols.map(async (symbol) => {
          ...
          this.logger.debug(`Metal price for ${symbol} fetched from API and cached`);
    ...
    this.on('READ', RelatedAssets, async (req) => {
      ...
      const [relatedAssets, err] = await Parqet.getRelatedAssets(parsingResult.data, 6);
      if (err) {
        req.error(err);
        return;
      }
      ...
      const shouldExpandQuotes = '$expand' in reqQuery && reqQuery.$expand === 'quotes';
    ...
    this.on('READ', Assets, async (req) => {
      ...
      const rawIdentifiers = Array.isArray(reqQuery.identifier)
        ? reqQuery.identifier
        : [reqQuery.identifier];
      const identifiers = z.array(AssetIdentifier).safeParse(rawIdentifiers);
      ...
      const assetPromises: Promise<z.infer<typeof BackendSchemas.Asset> | null>[] =
        identifiers.data.map(async (identifier) => {
          ...
          const securityType: z.infer<typeof BackendSchemas.Asset>['securityType'] =
            assetType === 'Security'
              ? assetDetails.asset.security.type
              : assetType === 'Crypto'
                ? 'Crypto'
                : 'Commodity';
          const assetSecurity = assetType === 'Security' ? assetDetails.asset.security : null;
          ...
          const marketCap = assetDetails.details.securityDetails?.marketCap || null;
          ...
          const peRatioTTM = assetDetails.details.securityDetails?.peRatioTTM || null;
          const pegRatioTTM = assetDetails.details.securityDetails?.pegRatioTTM || null;
          const priceFairValueRatio =
            assetDetails.details.securityDetails?.priceFairValueTTM || null;
          const dividendYieldPercentageTTM =
            assetDetails.details.securityDetails?.dividendYielPercentageTTM || null;
          const dividendPerShareTTM =
            assetDetails.details.securityDetails?.dividendPerShareTTM || null;
          const payoutRatioTTM = assetDetails.details.securityDetails?.payoutRatioTTM || null;
          const fiftyTwoWeekRange = assetDetails.details.securityDetails?.fiftyTwoWeekRange || null;
          const dividendPayoutInterval = assetDetails.details.payoutInterval ?? 'none';
          const historicalDividends = assetDetails.details.historicalDividends || [];
          ...
          const combinedEtfDetails: z.infer<typeof BackendSchemas.Asset>['etfDetails'] =
          ...
          const result: z.infer<typeof BackendSchemas.Asset> = {
            identifier: assetDetails.asset._id.identifier,
            wkn:
              assetDetails.asset.assetType === 'Security' ? assetDetails.asset.security.wkn : null,
            ...
            fiftyTwoWeekRange,
            financials: {
              annual: assetDetails.details.securityDetails?.annualFinancials || [],
              quarterly: assetDetails.details.securityDetails?.quarterlyFinancials || [],
              incomeStatementGrowth:
                assetDetails.details.securityDetails?.incomeStatementGrowth || [],
            },
            symbols:
              assetSecurity && assetSecurity.type !== 'Zertifikat' ? assetSecurity?.symbols : [],
            dividends: {
              payoutInterval: dividendPayoutInterval as z.infer<
                typeof ParqetSchemas.DividendPayoutInterval
              >,
              ...
              future: futureDividends.map((dividend) => ({
                ...
                recordDate: dividend.recordDate instanceof Date ? dividend.recordDate : null,
                exDate: dividend.exDate,
                isEstimated: dividend.isEstimated,
              })),
            ...
            analysis: {
              ...
              priceTargetConsensus: assetDetails.details.priceTargetConsensus || null,
      ...
      return (
        assets.filter((asset): asset is z.infer<typeof BackendSchemas.Asset> => asset !== null) ||
        []
      );
    });
  ...
  private async getSecurityMapping(cacheKey: 'sectors' | 'regions' | 'industries') {
  ...
  private async handleSecurityMappingRequest(
    request: cds.Request<
      typeof SecurityIndustries | typeof SecurityRegions | typeof SecuritySectors
    >
  ) {
    try {
