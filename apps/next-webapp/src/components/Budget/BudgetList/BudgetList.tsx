...
export const BudgetList: React.FC<BudgetListProps> = ({}) => {
  ...
  const handleFormSubmission: EntityDrawerFormHandler<EntityFormFields> = async (
    ...
    if (action === 'CREATE') {
      ...
      showSnackbar({ message: `Budget '${parsedPayload.data.name}' created successfully` });
      dispatchDrawerAction({ type: 'CLOSE' });
      onSuccess?.();
      dispatch(refresh());
    } else if (action === 'EDIT') {
      ...
      const [_, error] = await BudgetService.update(entityId, parsedPayload.data);
      if (error) {
        return showSnackbar({
          message: `Failed to create budget: ${error.message}`,
          action: <Button onClick={() => handleFormSubmission(payload, onSuccess)}>Retry</Button>,
      ...
      <Card>
        ...
        <Card.Body>
          ...
          {status === 'loading' ? (
            ...
            <Stack rowGap={1}>
              {budgets.map(({ ID, name, budget, toCategories, type, balance }) => {
                return (
                  <BudgetItem
                    key={ID}
                    budget={{
                      ...
                      type,
                      budget,
                      categories: toCategories.map(({ toCategory: { ID, name, description } }) => ({
                        ID,
                        name,
                        description,
                      })),
