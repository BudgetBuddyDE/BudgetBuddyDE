...
export const DividendInformation: React.FC<DividendInformationProps> = ({
	...
	const kpiEntries = Object.entries(KPIs ?? {}).map(([key, value]) => ({
		key,
		value: value ?? 0,
	}));
				...
				<Accordion disabled={!future.length}>
					...
					<AccordionDetails sx={{ px: 0 }}>
						<List dense disablePadding sx={{ py: 0 }}>
							{future.map(
								(
									{ exDate, price, paymentDate, currency, isEstimated },
									idx,
									arr,
								) => {
									return (
												...
												<ListItemText
													primary={
														<Stack>
															<Typography>
																{"Ex-Date: " +
																	Formatter.date.format(exDate, true)}
															</Typography>
															<Typography>
																{"Fullfillment: " +
																	Formatter.date.format(paymentDate, true)}
															</Typography>
														</Stack>
													}
												/>
							...
							{yearlyTTM?.reverse().map(({ year, dividend }, idx, arr) => (
								<React.Fragment key={year}>
									<ListItem
										...
										secondaryAction={
											<Typography>
												{Formatter.currency.formatBalance(dividend)}
											</Typography>
										}
									>
