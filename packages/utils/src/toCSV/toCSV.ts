...
export type TransformFieldFunc<T extends object, R> = (
	value: T[keyof T],
	item: T,
	list: T[],
	index: number,
...
export function toCSV<T extends object>(
	...
	const headerFields = fields.map((field) =>
		typeof field === "object" && "as" in field ? field.as : field,
	);
		...
		.map((item, index) =>
			fields
				.map((field) => {
					...
					return FIELD_IS_OBJ && field.transform
						? field.transform(fieldVal, item, arr, index)
						: fieldVal;
				})
				.join(separator),
		)
