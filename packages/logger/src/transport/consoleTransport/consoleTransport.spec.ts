...
describe("ConsoleTransport", () => {
	...
	describe("custom formatting", () => {
		test("should use custom format function when provided", () => {
			...
			expect(consoleLogSpy).toHaveBeenCalledWith(
				"[CUSTOM-INFO] custom-format-test: Custom formatted message",
			);
		});
		...
		test("should pass metadata to custom format function", () => {
			...
			expect(consoleWarnSpy).toHaveBeenCalledWith(
				'[CUSTOM] Request failed [Meta: {"requestId":"abc123"}]',
				metadata,
			);
		});
		...
		test("should handle custom format function with different log levels", () => {
			...
			expect(consoleWarnSpy).toHaveBeenCalledWith(
				"🚀 [WARN] Warning with emoji",
			);
			expect(consoleErrorSpy).toHaveBeenCalledWith(
				"🚀 [ERROR] Error with emoji",
			);
		});
	});
	...
	describe("batching and debouncing", () => {
		...
		test("should handle debouncing correctly", () => {
			...
			expect(consoleLogSpy).toHaveBeenNthCalledWith(
				1,
				expect.stringContaining("Debounced message 1"),
			);
			expect(consoleLogSpy).toHaveBeenNthCalledWith(
				2,
				expect.stringContaining("Debounced message 2"),
			);
		});
	...
	describe("edge cases", () => {
		test("should handle empty message", () => {
			...
			expect(consoleLogSpy).toHaveBeenCalledWith(
				expect.stringContaining("[empty-message-test]"),
			);
		});
		...
		test("should handle message with parameters", () => {
			...
			expect(consoleLogSpy).toHaveBeenCalledWith(
				expect.stringContaining("User John has 42 points"),
			);
		});
		...
		test("should handle complex metadata objects", () => {
			...
			expect(consoleLogSpy).toHaveBeenCalledWith(
				expect.stringContaining("Complex operation completed"),
				{
					request:
						'{"method":"POST","url":"/api/users","headers":{"content-type":"application/json"}}',
