...
import { type TCategory, type TExpandedSubscription } from '@/types';
...
const SUBSCRIPTION_TYPES: readonly SubscriptionType[] = ['INCOME', 'EXPENSE'] as const;

const SUBSCRIPTION_TYPE_META: Record<SubscriptionType, { label: string; emptyText: string }> = {
  INCOME: {
    label: 'Income',
...
type Action =
  ...
  | { type: 'success'; subscriptionType: SubscriptionType; payload: CategoryStats[] }
...
function reducer(state: State<SubscriptionType>, action: Action): State<SubscriptionType> {
  switch (action.type) {
    case 'start':
      return {
        ...state,
...
export const SubscriptionPieChart: React.FC<SubscriptionPieChartProps> = ({
  ...
  const [subscriptionType, setSubscriptionType] = React.useState<SubscriptionType>('INCOME');
  ...
  const fetchData = React.useCallback(
    async (type: SubscriptionType) => {
      ...
      try {
        ...
        dispatch({ type: 'success', subscriptionType: type, payload: categoryStats });
      } catch (err) {
  ...
  const renderContent = () => {
    ...
    if (chartData.length === 0)
      return <NoResults text={SUBSCRIPTION_TYPE_META[subscriptionType].emptyText} />;
      ...
      <Card.Header>
        ...
        <Card.HeaderActions sx={{ display: 'flex', flexDirection: 'row' }}>
          <ToggleButtonGroup
            ...
            onChange={(_, value: SubscriptionType) => setSubscriptionType(value)}
            exclusive
          >
...
function groupSubscriptionsByCategory(subscriptions: TExpandedSubscription[]): CategoryStats[] {
