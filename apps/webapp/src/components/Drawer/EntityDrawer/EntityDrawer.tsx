...
export const EntityDrawer = <T extends FieldValues>({
  ...
  const renderedFields: JSX.Element[] = React.useMemo(() => {
    return fields.map(field => {
      ...
      switch (field.type) {
        case 'date':
          return <DateFieldComponent key={field.name} field={field} control={form.control} wrapperSize={wrapperSize} />;

        case 'text':
          return <TextFieldComponent key={field.name} field={field} control={form.control} wrapperSize={wrapperSize} />;

        case 'number':
          return (
            <NumberFieldComponent key={field.name} field={field} control={form.control} wrapperSize={wrapperSize} />
          );
...
export type {EntityDrawerField, FirstLevelNullable} from './types';
 ...
 */
function preProcessFormPayload<T extends FieldValues>(formData: T, fields: EntityDrawerField<T>[]): T {
