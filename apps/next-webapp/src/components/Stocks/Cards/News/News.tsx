    ...
    <Card sx={{ p: 0 }}>
      <Card.Header sx={{ px: 2, pt: 2, mb: 0 }}>
        <Box>
          ...
          <Card.Subtitle>{hasNews ? `Latest news updates` : `No news available`}</Card.Subtitle>
        </Box>
      </Card.Header>
      <Card.Body sx={{ p: hasNews ? 0 : 2 }}>
        {!hasNews && <NoResults icon={<NewspaperRounded />} text={'No news available'} />}
                  ...
                  <ListItemText
                    primary={
                      <Typography variant="caption" color="textSecondary">
                        {Formatter.date.formatWithPattern(entry.publishedAt, 'dd.MM.yy HH:mm')}
                      </Typography>
                    }
                    secondary={
                      <React.Fragment>
