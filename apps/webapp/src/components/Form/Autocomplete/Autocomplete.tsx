...
>) => {
	...
	const handleOpen = () => {
		...
		if (searchAsYouType) {
			logger.debug(
				"searchAsYouType is enabled, skipping fetching options on open",
			);
			return;
		}
	...
	useEnhancedEffect(() => {
		if (!inputValue) {
			logger.warn(
				"Current inputValue is empty, skip fetching and returning empty options",
			);
			setOptions([]);
			return;
		}
		...
		logger.debug(
			"Input value changed, fetching options with keywords: %s",
			inputValue,
		);
		void fetchOptions();
	}, [inputValue]);
			...
			onInputChange={
				searchAsYouType
					? // REVISIT: Still triggers quote often when typing longer words which is resultung in many requests
						debounce(
							(_, value) => setInputValue(value),
							props.debounceInMillis ?? 300,
						)
					: undefined
			}
