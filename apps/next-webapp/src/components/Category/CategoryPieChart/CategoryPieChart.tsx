...
export function CategoryPieChart({
	...
	const isLoading =
		Boolean(state.loading[currentTimeframe]) ||
		(!stats && !state.error[currentTimeframe]);
					...
					>
						{TIMEFRAMES.map((tf) => (
							<ToggleButton
								key={tf}
								value={tf}
								aria-label={TIMEFRAME_META[tf].label}
							>
			...
			<Card.Body sx={{ pt: 1 }}>
				{isLoading ? (
					...
					<PieChart
						...
						secondaryText={
							transactionsType === "EXPENSE" ? "Expenses" : "Income"
						}
						series={[
							{
								data: chartData,
...
function toPieData(
	...
	return stats
		...
		.filter((d) =>
			Number.isFinite(d.value) ? (d.value as number) > 0 : false,
		);
}
