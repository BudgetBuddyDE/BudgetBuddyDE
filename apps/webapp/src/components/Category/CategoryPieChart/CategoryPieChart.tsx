 ...
 * - Empty-state text
 */
const TIMEFRAME_META: Record<CategoryPieChartTimeframe, {label: string; range: () => [Date, Date]; emptyText: string}> =
  {
    MONTH: {
      label: 'Month',
      range: () => {
...
export function CategoryPieChart({
  ...
  const handleTimeframeChange = (_: React.MouseEvent<HTMLElement>, next: CategoryPieChartTimeframe | null) => {
    if (!next || next === currentTimeframe) return;
    setCurrentTimeframe(next);
  };

  const stats = state.data[currentTimeframe];
  const isLoading = Boolean(state.loading[currentTimeframe]) || (!stats && !state.error[currentTimeframe]);
  const error = state.error[currentTimeframe];

  const chartData = React.useMemo(() => (stats ? toPieData(stats, transactionsType) : []), [stats, transactionsType]);
 ...
 */
function toPieData(stats: Stats[], type: CategoryPieChartTransactionType): PieChartData[] {
