...
import type {IGetHistoricalBalanceQuery, IGetInsightsReportQuery} from '../types/interfaces';
import {
  GetHistoricalBalanceResponse,
  GetHistoricalCategoryBalanceResponse,
  GetInsightsReportResponse,
} from '../types/schemas';
...
export class InsightsService extends BackendService {
  ...
  @log
  async getReport<Query extends IGetInsightsReportQuery>(
    query: Query,
    requestConfig?: RequestInit,
  ): Promise<TResult<TypeOfSchema<typeof GetInsightsReportResponse>>> {
    return this.requestJson(
      `${this.getBaseRequestPath()}/report?${this.reqQueryObjToURLSearchParams(query).toString()}`,
      {
        method: 'GET',
        headers: new Headers(requestConfig?.headers || {}),
        credentials: 'include',
