...
import type { AssetIdentifier } from "@budgetbuddyde/types";
...
export const StockPositionTable: React.FC<StockPositionTableProps> = ({
			...
			return [
				...
				},
				{
					...
					async retrieveOptionsFunc() {
						const [stockExchanges, error] =
							await AssetService.exchange.getValueHelps();
						if (error) {
					...
					isOptionEqualToValue(
						option: TStockExchangeVH,
						value: TStockExchangeVH,
					) {
				...
				},
				{
					...
					async retrieveOptionsFunc(fieldText?: string) {
						...
						const [searchResults, error] = await AssetService.positions.search(
							fieldText || "",
						);
						if (error) {
							logger.error(
	...
	}, [dispatch, getPage, currentPage, rowsPerPage, assets.length, assets.map]);
	...
	return (
		<React.Fragment>
			<EntityTable<TExpandedStockPosition>
				...
				rowHeight={83.5}
				renderRow={(_cell, position, _list) => {
								...
								<Stack sx={{ textAlign: "right" }}>
									<Typography
										sx={{
											...
											color: (theme) =>
												theme.palette[
													position.absoluteProfit >= 0 ? "success" : "error"
												].main,
										}}
									>
										{Formatter.currency.formatBalance(
											position.absoluteProfit,
											currency,
										)}
									</Typography>
									<Typography
