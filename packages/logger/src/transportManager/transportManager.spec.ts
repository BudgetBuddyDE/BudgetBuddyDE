...
describe("TransportManager", () => {
	...
	describe("add", () => {
		...
		test("should allow adding the same transport multiple times", () => {
			...
			expect(
				transportManager.registeredTransports.filter((t) => t === transport1),
			).toHaveLength(2);
		});
	});
	...
	describe("integration scenarios", () => {
		...
		test("should handle manager initialization with transports", () => {
			const preInitializedManager = new TransportManager([
				transport1,
				transport2,
			]);
		...
		test("should handle error conditions gracefully", () => {
			...
			expect(() =>
				transportManager.addLogToQueue(createLogEntry()),
			).not.toThrow();
