...
const formatMessage = (level: LogLevel, message: string, label: string): string => {
  ...
  return `${colors.dim}${timestamp}${colors.reset} ${levelColors[level]}${level.toUpperCase()}${
    colors.reset
  } ${colors.bright}[${label}]:${colors.reset} ${message}`;
};

type LoggerClient = Record<LogLevel, (...params: LogHandlerParams) => void>;

export const createLogger = (
  options?: Logger
): LoggerClient & {
  childLogger: (childLoggerLabel: string) => LoggerClient;
} => {
  const enabled = options?.disabled !== true;
  const logLevel = options?.level ?? 'error';
  const label = options?.label ?? 'yfinance';
