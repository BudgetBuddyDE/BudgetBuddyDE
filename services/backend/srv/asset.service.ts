...
export class AssetService extends BaseService {
  ...
  async init() {
    ...
    this.on("READ", Assets, async (req) => {
      ...
      return (
        assets.filter(
          (asset): asset is z.infer<typeof BackendSchemas.Asset> =>
            asset !== null,
        ) || []
      );
    });
  ...
  private async getSecurityMapping(
    ...
    let fetchedData: z.infer<
      | typeof ParqetSchemas.Sector
      | typeof ParqetSchemas.Region
      | typeof ParqetSchemas.Industry
    >[] = [];
    switch (cacheKey) {
      case "sectors": {
