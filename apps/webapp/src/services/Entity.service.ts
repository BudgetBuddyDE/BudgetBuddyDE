...
> {
  ...
  protected schemas: EntitySchemas<GetAllResult, GetResult, CreateResult, UpdateResult, DeleteResult>;
  protected logger = logger.child({label: this.constructor.name});

  constructor(
    basePath: string = '',
    schemas: EntitySchemas<GetAllResult, GetResult, CreateResult, UpdateResult, DeleteResult>,
  ) {
    this.host = process.env.NEXT_PUBLIC_BACKEND_SERVICE_HOST || 'http://localhost:9000';
  ...
  protected handleZodError<T, S>(errors: z.ZodError<S>[] | z.ZodError<S>): ServiceResponse<T> {
  ...
  async getAll<Q extends BaseGetAllQuery>(
    ...
    try {
        ...
        Object.entries(query).forEach(([key, value]) => {
          queryParams.append(
            ...
            (value as unknown) instanceof Date
              ? Formatter.date.formatWithPattern(value as unknown as Date, 'yyyy-MM-dd')
              : String(value),
          );
        });
      }
      ...
      if (!response.ok) {
        this.logger.error('Response not OK for getAll (requestID=%s): %s', await response.text(), {requestId});
  ...
  async getById(entityId: string, requestConfig?: RequestInit): Promise<ServiceResponse<z.output<GetResult>>> {
    try {
      const response = await fetch(
  ...
  async create(payload: CreatePayload, requestConfig?: RequestInit): Promise<ServiceResponse<z.output<CreateResult>>> {
    try {
      const response = await fetch(
  ...
  async deleteById(entityId: string, requestConfig?: RequestInit): Promise<ServiceResponse<z.output<DeleteResult>>> {
    try {
      const response = await fetch(
        `${this.getBaseRequestPath()}/${entityId}`,
        this.mergeRequestConfig(
