...
export function createLogger(options: LogClientOptions): LogClient {
	...
	const defaultMeta = options.defaultMeta
		? sanitizeLogMeta(options.defaultMeta)
		: undefined;
	...
	return {
		...Object.fromEntries(
			LEVEL_STRINGS.map((level) => [
				level,
				log(LogLevel[level.toUpperCase() as LogLevelStrings]),
			]),
		),
		...
		child(childOptions) {
			const childTransports: Transport[] =
				childOptions.transports || options.transports || [];
			const mergedOptions: LogClientOptions = {
				...
				supressNoTransportWarning:
					childOptions.supressNoTransportWarning ??
					options.supressNoTransportWarning,
			};
...
export function splitLogParams(args: any[]): {
	msg: string;
	// biome-ignore lint/suspicious/noExplicitAny: Will be fixed in future version (for sureee)
	params: any[];
	meta?: LogMeta;
} {
