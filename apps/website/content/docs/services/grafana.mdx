...
For this, a pre-configured setup with multiple services is used, including Prometheus as a data source for metrics, Loki for logs, and Tempo for traces.
...
Ports:
- `3200`: HTTP Server for queries
- `4317`: gRPC Ingest (OTLP)
- `4318`: HTTP Ingest (OTLP)

Traces are stored locally under `/var/tempo/traces`.
