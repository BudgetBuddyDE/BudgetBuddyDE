...
const Card = forwardRef<HTMLDivElement, CardProps>(({ children, sx, ...props }, ref) => (
  <Paper ref={ref} elevation={3} sx={{ p: 2, borderRadius: 2, ...sx }} {...props}>
    {children}
  </Paper>
));
...
const Title = forwardRef<HTMLSpanElement, CardSectionProps>(({ children, sx, ...props }, ref) => (
  <Typography ref={ref} variant="subtitle1" sx={sx} fontWeight={'bold'}>
    {children}
  </Typography>
));
Title.displayName = 'CardTitle';

const Subtitle = forwardRef<HTMLSpanElement, CardSectionProps>(
  ({ children, sx, ...props }, ref) => (
    <Typography ref={ref} variant="subtitle2" color="text.secondary" sx={sx}>
      {children}
    </Typography>
  )
);
