...
export type FinancialsAccordionProps = Pick<FinancialStatementsProps, 'currency' | 'financials'>;

export const FinancialsAccordion: React.FC<FinancialsAccordionProps> = ({
  currency,
  financials,
}) => {
  const screenSize = useScreenSize();
  const [currentTab, setCurrentTab] = React.useState<'quarterly' | 'annual'>('annual');
  ...
  return (
    <Accordion disabled={!chartData.length}>
      <AccordionSummary expandIcon={<ExpandMoreRounded />}>
        <Typography component="span">Financial Statements</Typography>
      </AccordionSummary>
      <AccordionDetails sx={{ px: 0 }}>
        <Box>
          <Tabs value={currentTab} onChange={(_, value) => setCurrentTab(value)} sx={{ mx: 2 }}>
        ...
        <Box>
          <BarChart
            ...
            yAxis={[
              {
                valueFormatter: (value: any) => Formatter.currency.shortenNumber(value ?? 0),
                width: 80,
              },
            ]}
            xAxis={[
              {
                scaleType: 'band',
                data: chartData.map(({ date }) => Formatter.date.format(date)).reverse(),
