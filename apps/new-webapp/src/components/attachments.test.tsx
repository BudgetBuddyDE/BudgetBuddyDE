...
const api = vi.hoisted(() => ({getAll: vi.fn(), upload: vi.fn(), remove: vi.fn()}));
vi.mock('@/apiClient', () => ({
  apiClient: {
    backend: {
      transaction: {getAllTransactionAttachments: api.getAll, uploadTransactionAttachments: api.upload},
...
describe('Attachments', () => {
  it('loads the signed gallery in chronological batches', async () => {
    ...
    expect(screen.getByText(/signed URLs expire after 5 min/i)).toBeVisible();
  ...
  it('validates upload file types before contacting the API', async () => {
    ...
    await userEvent.upload(screen.getByLabelText(/choose receipt images/i), oversizedImage);
    await userEvent.click(screen.getByRole('button', {name: 'Upload'}));
    expect(await screen.findByRole('alert')).toHaveTextContent(/not a supported image/i);
