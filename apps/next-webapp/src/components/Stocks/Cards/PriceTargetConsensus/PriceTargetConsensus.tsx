...
export type PriceTargetConsensusProps = {
  currency: string;
  priceTargetConsensus: Omit<TAsset['analysis']['priceTargetConsensus'], 'currency'> | null;
};
...
export const PriceTargetConsensus: React.FC<PriceTargetConsensusProps> = ({
  ...
  const converted = Object.entries(priceTargetConsensus ?? {}).map(([key, value]) => ({
    key,
          ...
          <List dense disablePadding sx={{ py: 0 }}>
            {converted.map(({ key, value }, idx, arr) => (
              <React.Fragment key={key}>
                <ListItem
                  secondaryAction={
                    <Typography>{Formatter.currency.formatBalance(value, currency)}</Typography>
                  }
                >
                  <ListItemText
                    primary={
