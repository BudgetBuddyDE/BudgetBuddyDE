...
<Callout type="warn">
  Note that the services in this setup are not additionally secured, as they are not exposed and
  only operate within your own network. If you plan to expose these services, it is important to
  secure them (e.g., using a reverse proxy) to prevent unauthorized access or file manipulation by
  third parties.
</Callout>
